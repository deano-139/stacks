...
import { loadUsers, saveUsers, saveBest } from './storage.js';
import { showGameOver, updateHUD } from './ui.js';

export function initScene() {
  state.stack = [{ x:0, z:0, sx:CONFIG.BASE, sz:CONFIG.BASE, hue: hueFor(0) }];
  state.moving = null; state.debris = []; state.particles = []; state.floaters = [];
  state.camY = 0; state.camTarget = 0; state.score = 0; state.combo = 0;
...
export function spawnMoving() {
  ...
  if (axis === 'x') b.x = side * CONFIG.MOVE_LIMIT; else b.z = side * CONFIG.MOVE_LIMIT;
  return b;
}
...
function addFloater(text, b, level, size) {
  state.floaters.push({ text, wx: b.x, wz: b.z, wy: level + 1.3, t: 0, dur: 0.95, size });
}

export function drop() {
  if (!state.moving) return;
  const b = state.moving, prev = state.stack[state.stack.length - 1];
  const axis = b.axis;
  ...
  if (Math.abs(delta) < CONFIG.EPS) {
    ...
    state.flash = 1;
    burst(b, level);
    addFloater(state.combo >= 2 ? `PERFECT ×${state.combo}` : 'PERFECT!', b, level,
    ...
    if (axis === 'x') {
      state.debris.push({ x: cutC, z: b.z, sx: cutSize, sz: b.sz, hue: b.hue,
        y: level, vy: 0, vx: sgn * 1.6, vz: 0, vr: sgn * 2.2, rot: 0 });
      b.x = newC; b.sx = newSize;
    } else {
      state.debris.push({ x: b.x, z: cutC, sx: b.sx, sz: cutSize, hue: b.hue,
  ...
  state.stack.push({ x: b.x, z: b.z, sx: b.sx, sz: b.sz, hue: b.hue, growAnim: b.growAnim });
  state.score++;
  updateHUD();
...
function die() {
  ...
  if (state.currentUser) {
    ...
    if (u) {
      u.plays = (u.plays || 0) + 1;
      if (state.score > (u.best || 0)) { u.best = state.score; isNew = true; }
      saveUsers(users);
      state.best = u.best;
    }
  } else {
    isNew = state.score > state.best;
    if (isNew) { state.best = state.score; saveBest(state.best); }
  }
  updateHUD();
